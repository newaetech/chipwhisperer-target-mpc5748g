...
void jitterbug(void)
{
static uint8_t jitterindex;
uint8_t max;
max = (sbox[jitterindex]) & 0x03;
for(volatile uint8_t k = 0; k < max; k++);
jitterindex++;
}
...
static void SubBytes(void)
{
  ...
  //Example of setting trigger high inside function - uncomment this,
  //and comment out the TRIG_HIGH in the main function
  //SIUL2.GPDO[PI6].B.PDO_4n = 1;
  ...
  //Can use this for timing information too
  //SIUL2.GPDO[PI6].B.PDO_4n = 0;
}
...
void AES128_ECB_tinyaes_setkey(uint8_t* key)
{
  Key = key;
  KeyExpansion();
}

void AES128_ECB_tinyaes_crypto(uint8_t* input)
{
  state = (state_t*)input;
  Cipher();
}
